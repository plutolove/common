...
ExceptionBase::ExceptionBase(const std::string& msg, const std::string& arg, int code)
    : _msg(msg), _pNested(0), _code(code) {
  if (!arg.empty()) {
    _msg.append(": ");
    _msg.append(arg);
...
ExceptionBase::ExceptionBase(const std::string& msg, const ExceptionBase& nested, int code)
...
ExceptionBase::~ExceptionBase() noexcept { delete _pNested; }
...
const char* ExceptionBase::name() const noexcept { return "ExceptionBase"; }

const char* ExceptionBase::className() const noexcept {
  return typeid(*this).name();
}

const char* ExceptionBase::what() const noexcept { return name(); }
